...
code can help you more clearly define the APIs, interfaces, and classes: what the code is doing and why. It can be sort of like "rubber duck debugging,"
describing the code can help you clarify your intent.

I will probably expand this into a few different posts covering various 
...
to spot real problems when there are 50 different warnings and suggestions in every file.

To get started, this first pass involved simply adding file headers. StyleCop has a default
file header template that you can modify as needed. It also has rules (`SA16xx`) to let you 
know if a file is missing a header or if the header is incorrect or improperly formatted. 
...
The next step was to add XML documentation comments. This falls under the 
`SA1600` and other rules. I'll discuss the meanings of these rule names/numbers below. 
Basic method documentation comments are pretty straightforward. You have a summary,
parameters, and a return value. Remarks and exceptions can be added as needed. In this pass,
...
Many of these "rules" are more like "best practices" or "suggestions." These rules can be configured to be 
...
Another step, ongoing during the first two, was to format namespaces (block vs. file-scoped)
...
any "missing" usings or unnecessary usings. The "missing" usings are ones
that are not needed for the code to compile, but are used in the XML documentation. 
...
For example, I don't care about the ordering of private, protected, and public members 
or types of private variables. I care more about grouping related members together.
